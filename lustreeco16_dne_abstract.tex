...
Our test environment used Arthur, a single Cray XK7 cabinet with
\todo{TO-DO: XYZ??} nodes. At the time of DNE P1 tests, Arthur nodes were running the Cray CLE 5.2.UP02
...
the DNE P1. In standard mdtest, each iteration of test creates a new
directory {\footnotesize{\texttt{test-dir}}}, which always results in testing the non-DNE case. To
work around this problem mdtest was modified to detect and use existing
test directories. Standard mdtest also iterates sequentially over each target
directory, but since each directory is located on a single MDT
with each being on a different server we can not evaluate the performance gain
of DNE P1 in that setup. Thus, the second mdtest modification required was to allow running all
test directory iterations at the same time in parallel. 
%The mdtest benchmark execution script can be found in Appendix A. The modified
%mdtest source code changes will be merged back into the publicly available
%mdtest soon.

For DNE P1 testing, we used 1,000 files per directory as a constant MDS
directory load. Tests were carried out during October of 2014. We used nineteen client nodes
on Arthur with 32 tasks per node, for a total of 608 tasks. Each test was run
ten times. 

%A test sample output is given below:
...
%The mdtest benchmark execution script can be
...
%Sarp: This is probably not needed but I am making a dump from the tech report,
%and I am keeping this for the time being for the sake of completeness. Feel
%free to knock this one down.
